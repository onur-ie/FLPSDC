...
void ModelPopulate (){
	...
	p = create_double_vector(J * K);
	q = create_double_vector(J * K);
...
//	Note:	 'G', 'L', 'E', and 'R', indicating greater-than, less-than, equality, and ranged constraints, respectively.
	...
	for (j = 0; j < J; j++) {
		for (k = 0; k < K; k++) {
			...
			//	if (x[(i * J + j) + ( k * I * J )] >= 0){
			//		rmatind[nzcnt] = x[(i * J + j) + ( k * I * J )];
			//		rmatval[nzcnt++] = lambda[i] - lambda[i] ;
			//	}
			//}
			if (p[j * K + k] >= 0){
				rmatind[nzcnt] = p[j * K + k];
				rmatval[nzcnt++] = 1;
			}
			if (s[j * K + k] >= 0){
	...
	for (j = 0; j < J; j++) {
		for (k = 0; k < K; k++) {
			...
			if (p[j * K + k] >= 0){
				qmatind[nzcnt] = p[j * K + k];
				qmatval[nzcnt++] = 1;
			}
			if (q[j * K + k] >= 0){
				qmatind[nzcnt] = q[j * K + k];
				qmatval[nzcnt++] = -1;
			}
			if (nzcnt >= 100000) {
				sprintf(errmsg, "MAXNZ exceeded, constraint n. %d \n", constraints);
