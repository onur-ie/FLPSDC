...
int * pos_X, * pos_Y, * pos_s, * pos_V, * pos_d, * pos_dv, * pos_tempX, * pos_Z, * pos_L;
double * pi, * sigma;
...
double MISOCP(void)
{
	...
	pos_s = create_int_vector(J * K);
	pos_d = create_int_vector(J * K);
	pos_V = create_int_vector(J * K);
	pos_dv = create_int_vector(J * K);
	...
	for (i = 0; i < I; i++) {
		for (j = 0; j < J; j++) {
			for (k = 0; k < K; k++) {
				...
				obj[index1] = (lambda[i] * t[i * J + j] * at) + ((aw * lambda[i] * (1 - cv[j * K + k] * cv[j * K + k])) / (2 * mu[j * K + k]));
				ctype[index1] = 'C';
				lbC[index1] = 0;
				ub[index1] = 1;
				index1++;
	...
	for (j = 0; j < J; j++) {
		for (k = 0; k < K; k++) {
			pos_s[j * K + k] = num_Y_var + num_x_var + index1;
			obj[index1] = (aw * (1 + (float)cv[j * K + k] * cv[j * K + k])) / 2;
			ctype[index1] = 'C';
			lbC[index1] = 0;
	...
	for (j = 0; j < J; j++) {
		for (k = 0; k < K; k++) {
			pos_V[j * K + k] = num_Y_var + num_x_var + index1 + num_d_var + num_s_var;
			obj[index1] = 0;
			ctype[index1] = 'C';
			lbC[index1] = 0;
			ub[index1] = CPX_INFBOUND;
	...
	for (j = 0; j < J; j++) {
		for (k = 0; k < K; k++) {
			pos_dv[j * K + k] = num_Y_var + num_x_var + index1 + num_d_var + num_s_var + num_V_var;
			obj[index1] = 0;
			ctype[index1] = 'C';
			lbC[index1] = 0;
			ub[index1] = CPX_INFBOUND;
	...
	for (j = 0; j < J; j++) {
		for (k = 0; k < K; k++) {
			pos_tempX[j * K + k] = num_Y_var + num_x_var + index1 + num_d_var + num_s_var + num_V_var + num_dv_var;
			obj[index1] = 0;
			ctype[index1] = 'C';
			lbC[index1] = 0;
			ub[index1] = CPX_INFBOUND;
	...
	for (j = 0; j < J; j++) {
		for (k = 0; k < K; k++) {
			pos_Z[j * K + k] = num_Y_var + num_x_var + index1 + num_d_var + num_s_var + num_V_var + num_dv_var + num_TX_var;
			obj[index1] = 0;
			ctype[index1] = 'C';
			lbC[index1] = 0;
			ub[index1] = CPX_INFBOUND;
	...
	for (j = 0; j < J; j++) {
		for (k = 0; k < K; k++) {
			pos_L[j * K + k] = num_Y_var + num_x_var + index1 + num_d_var + num_s_var + num_V_var + num_dv_var + num_TX_var, num_Z_var;
			obj[index1] = 0;
			ctype[index1] = 'C';
			lbC[index1] = 0;
			ub[index1] = CPX_INFBOUND;
	...
	for (j = 0; j < J; j++) {
		for (k = 0; k < K; k++) {
			...
			for (i = 0; i < I; i++) {
				matind[index] = pos_X[(i * J + j) + (k * I * J)];
				matval[index++] = -1 * lambda[i] * lambda[i]; 
			}
	...
	for (j = 0; j < J; j++) {
		for (k = 0; k < K; k++) {
				...
				qmatind[index] = pos_V[j * K + k];
				qmatind1[index] = pos_V[j * K + k];
				qmatval[index++] = 1;

				qmatind[index] = pos_dv[j * K + k];
				qmatind1[index] = pos_dv[j * K + k];
				qmatval[index++] = -1;
	...
	*useraction_p = CPX_CALLBACK_DEFAULT;				
	...
	for (j = 0; j < J; j++) {
		for (k = 0; k < K; k++) {
			...
			Sorted[counter].value = temp_x[pos_s[j * K + k]];
			counter++;
		}
	}
	...
	for (i = 1; i < J * K; i++) {
		if (Sorted[i].value > 0) {
			...
			temp1 = 0;
			for (j = 0; j <= i; j++) {
				temp += sigma[Sorted[i].index];
				if (i != 0)
					temp1 += sigma[Sorted[i - 1].index];
			}
			pi[Sorted[i].index] = temp - temp1;
			if (pi[Sorted[i].index] * Sorted[i].value > temp_x[pos_Z[Sorted[i].index]]) {
				numrows = 1;
				numnz = 2;
				i_vector(&matbeg, numrows, "open_cplex:4");
				i_vector(&matind, numnz, "open_cplex:6");
				...
				indextemp1 = index1;
				matbeg[index1++] = index;
				matind[index] = pos_Z[Sorted[i].index];
				matval[index++] = -1;
				matind[index] = pos_s[Sorted[i].index];
