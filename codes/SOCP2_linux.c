...
//	Note:	 'G', 'L', 'E', and 'R', indicating greater-than, less-than, equality, and ranged constraints, respectively.
	...
	for (j = 0; j < J; j++) {
		for (k = 0; k < K; k++) {
			...
			nzcnt = 0;
			rhs[0] = 0; //mu[j * K + k];  // meghdar un vare moadele
			sense[0] = 'L';
			rmatbeg[0] = 0;
			for (i = 0; i < I; i++) {
				if (x[(i * J + j) + ( k * I * J )] >= 0){
	...
	for (j = 0; j < J; j++) {
		for (k = 0; k < K; k++) {
			...
			if (s[j * K + k] >= 0){
				qmatind[nzcnt] = s[j * K + k];
				qmatval[nzcnt++] = 1;
			}
			if (dv[j * K + k] >= 0){
				qmatind[nzcnt] =dv[j * K + k];
				qmatval[nzcnt++] = -1;
			}
			if (nzcnt >= 100000) {
				sprintf(errmsg, "MAXNZ exceeded, constraint n. %d \n", constraints);
...
//CPXsetintparam(env, CPX_PARAM_MIQCPSTRAT, 2);
...
gettimeofday(&stop, NULL);
	...
	//cout<<endl<<"r:__________"<<endl;
	//////////////	 Print rho	 //////////////
	for (j = 0; j <J; j++) {
		for (k = 0; k <K; k++) {
			s[j * K + k] = X_Cplex[counter];
			//if (r[j * K + k] > 0)
			//	cout<<"r["<<j<<"]["<<k<<"] = "<<r[j * K + k]<<endl; 
			counter = counter + 1;
		}
	}
	...
	fprintf(Out, "%f\t", TT);
	fprintf(Out, "%f\t", WT);
	fprintf(Out, "%f\t", (double) (EndTime-StartTime)/ (double) CLOCKS_PER_SEC);
