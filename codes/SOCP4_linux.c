...
void ModelPopulate (){
	...
	for (j = 0; j <J; j++) {
		for (k = 0; k < K; k++) {
			//sprintf(colname[counter], "dv%3d_%3d", j, k);
			lb[counter] = 0;
			ub[counter] = M;
			obj[counter] =0;
			coltype[counter++] = 'C';
		...
		for (k = 0; k < K; k++) {
			...
			/*for (i = 0; i < I; i++) {
				if (x[(i * J + j) + ( k * I * J )] >= 0){
					...
					qmatval[nzcnt++] = 2 * lambda[i]*lambda[i];
				}
			}*/
			if (xtemp[j * K + k] >= 0){
