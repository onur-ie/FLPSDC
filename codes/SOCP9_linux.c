...
void ModelPopulate (){
	...
	w = create_double_vector(J * K);
	rr = create_double_vector(J * K);
...
//	Note:	 'G', 'L', 'E', and 'R', indicating greater-than, less-than, equality, and ranged constraints, respectively.
	...
	for (j = 0; j < J; j++) {
		for (k = 0; k < K; k++) {
			...
			qrhs = 1;  
			rmatbeg[0] = 0;
			qmatind[nzcnt] = rr[j * K + k];
			qmatind1[nzcnt] = w[j * K + k];
			qmatval[nzcnt++] = 1;
			if (nzcnt >= 100000) {
				sprintf(errmsg, "MAXNZ exceeded, constraint n. %d \n", constraints);
				printf("%s", errmsg);
			...
			if (nzcnt > 0) {
				//status = CPXaddrows(env, lp, 0, 1, nzcnt, rhs, sense, rmatbeg, rmatind, rmatval, NULL, NULL);
				status =  CPXaddqconstr(env,lp,0,nzcnt,qrhs,qsense0,NULL,NULL,qmatind,qmatind1,qmatval,NULL);
	...
	for (j = 0; j < J; j++) {
		for (k = 0; k < K; k++) {
			...
			qmatind[nzcnt] = y[j * K + k];
			qmatind1[nzcnt] = rho[j * K + k];
			qmatval[nzcnt++] = -1 * mu[j * K + k];
			if (nzcnt >= 100000) {
				sprintf(errmsg, "MAXNZ exceeded, constraint n. %d \n", constraints);
				printf("%s", errmsg);
				exit(-1);
	...
	for (i = 0; i < I; i++) {
		for (j = 0; j <J; j++) {
			for (k=0; k < K ; k++){
				...
				/*if (x[(i * J + j) + ( k * I * J )] != 0 ){
					cout<<"x["<<i<<"]["<<j<<"]["<<k<<"] = "<<x[(i * J + j) + ( k * I * J )]<<endl;
				}*/
...
fprintf(Out,"d:\t|\t");
	...
	for(j = 0; j < J; j++)
	{
		for(k = 0; k < K; k++)
		{
			if (rr[j * K + k] > 0) 
			{
				fprintf(Out, "%d\t%d\t=\t%f|\t", j, k, rr[j * K + k]);
			}
		}
	}
	...
	for(j = 0; j < J; j++)
	{
		for(k = 0; k < K; k++)
		{
			if (rho[j * K + k] > 0) 
			{
				fprintf(Out, "%d\t%d\t=\t%f|\t", j, k, rho[j * K + k]);
			}
		}
	}
